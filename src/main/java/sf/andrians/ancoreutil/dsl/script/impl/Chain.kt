...
class Chain(private val sibling: IScriptChild?, vararg a: CharSequence) : Stmt(*a) {
    override fun chain(vararg a: CharSequence): IStmt {
        return Chain(this, *a)
    }

    override fun renderTo(ret: StringBuilder, indent: String, tab: String) {
        sibling?.renderTo(ret, indent, tab)
        for (s in stmts) {
            ret.append(s)
        }
    }
