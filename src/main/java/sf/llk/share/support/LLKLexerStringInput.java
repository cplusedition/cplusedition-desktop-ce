...
public class LLKLexerStringInput extends AbstractLexerInput {
    ...
    private String laSource;
    private final int length;
    private boolean isIgnorecase = false;

    ////////////////////////////////////////////////////////////

    public LLKLexerStringInput(final String a, final ILLKMain main) {
        super(main);
        source = a;
        laSource = a;
    ...
    public Object clone() throws CloneNotSupportedException {
        return (LLKLexerStringInput) super.clone();
    }
    ...
    public final int consume(final IConsumer consumer) throws LLKParseError {
        for (; offset < length; ++offset) {
            if (!consumer.consume(laSource.charAt(offset))) {
                LA1x();
                return la1;
            }
        }
    ...
    public final int LA0() {
        ...
        return laSource.charAt(offset - 1);
    }
    ...
    public final boolean LA(final char[] expected) {
        ...
        for (int i = 0; i < len; ++i, ++index) {
            if (laSource.charAt(index) != expected[i]) {
                return false;
            }
        }
        return true;
    ...
    public void getChars(final int sstart, final int send, final char[] dst, final int dstart) {
        source.getChars(sstart, send, dst, dstart);
    }
    ...
    public void setIgnoreCase(final boolean ignorecase) {
        if (ignorecase) {
            if (!isIgnorecase) {
                isIgnorecase = true;
                laSource = source.toLowerCase(Locale.ROOT);
            }
        } else {
            isIgnorecase = false;
            laSource = source;
    ...
    public final void match(final char[] s) throws LLKParseException {
        ...
        for (int i = 0, index = offset; i < len; ++i, ++index) {
            c = (index >= length ? ILLKConstants.LEXER_EOF : laSource.charAt(index));
            if (c != s[i]) {
