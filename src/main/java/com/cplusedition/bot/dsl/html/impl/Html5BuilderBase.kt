...
abstract class Html5BuilderBase<E : IElement> : IHtml5Builder<E> {
    ...
    override fun xml(encoding: String): IPI {
        return PI("xml", "encoding=" + XMLUt.quoteAttr(encoding))
    }

    override fun xml(version: String, encoding: String): IPI {
        return PI("xml", "version=" + XMLUt.quoteAttr(version), "encoding=" + XMLUt.quoteAttr(encoding))
    }
    ...
    override fun esc(vararg content: String): IText {
        val c = content.map { XMLUt.esc(it).toString() }.toTypedArray()
        return Text(*c)
    }
    ...
    override fun pi(target: String, vararg content: String): IPI {
        return PI(target, content.bot.joinln())
    }
    ...
    override fun fragment(vararg children: Any): IFragment {
        val parent: IFragment = Fragment()
    ...
    override fun template(vararg children: Any): E {
        return elm("template", *children)
    }
