...
(function (global, factory) {
    ...
    var chromeOS = false;
    var windows = false;
    var presto_version = null;
    var flipCtrlCmd = false;
    var captureRightClick = false;
    //#END HACK
    //#BEGIN
    ...
    var dragAndDrop = function () {
        ...
        //#BEGIN HACK Drag and Drop
        //    if (ie && ie_version < 9) { return false }
        //    var div = elt('div');
        //    return "draggable" in div || "dragDrop" in div
        return false;
        //#END HACK Drag and Drop
    }();
    ...
    function buildLineContent(cm, lineView) {
        ...
        var content = eltP("span", null, null, webkit ? "padding-right: 1em" : null);
        var builder = {
    ...
    function onFocus(cm, e) {
        // console.log("# cm.onFocus");
        if (cm.state.delayingBlurEvent) { cm.state.delayingBlurEvent = false; }

        if (cm.options.readOnly == "nocursor") { return }
        // console.log("# cm.onFocus#1");
        if (!cm.state.focused) {
            // console.log("# cm.onFocus!focused");
    ...
    function onBlur(cm, e) {
        // console.log("# cm.onBlur");
        if (cm.state.delayingBlurEvent) { return }
        if (cm.state.focused) {
            // console.log("# cm.onBlur.focused");
            signal(cm, "blur", cm, e);
    ...
    var scrollbarModel = { "native": NullScrollbars, "null": NullScrollbars };
    //#END HACK
    ...
    function setSelection(doc, sel, options) {
        ...
        if (doc.cm) {
            signalLater(doc.cm, SELECTION_CHANGED, doc.cm)
        }
    }
    ...
    function registerEventHandlers(cm) {
        ...
        on(d.scroller, "touchend", function (e) {
            ...
            if (touch && !eventInWidget(d, e) && touch.left != null &&
                ...
                if (!touch.prev || farAway(touch, touch.prev)) // Single tap
                {
                    ...
                    //#BEGIN HACK Delay selectino for iOS 13.
        ...
        //#BEGIN HACK Prevent drag and drop.
        on(d.scroller, "dragstart", sinkhole);
        on(d.scroller, "dragenter", sinkhole);
        on(d.scroller, "dragover", sinkhole);
        on(d.scroller, "dragleave", sinkhole);
        on(d.scroller, "drop", sinkhole);
        //    d.dragFunctions = {
    ...
    function hiddenTextarea() {
        var te = elt("textarea", null, null, "position: absolute; bottom: -1em; padding: 0; width: 1px; height: 1em; border: none; outline: none");
        ...
        if (ios) { te.style.border = "1px solid transparent"; }
    ...
    function addEditorMethods(CodeMirror) {
        ...
        CodeMirror.prototype = {
            ...
            ensureFocus1: function () { ensureFocus(this); },
    ...
    ContentEditableInput.prototype.showSelection = function (info, takeFocus) {
        // console.log("# ContentEditableInput.showSelection");
    ...
    ContentEditableInput.prototype.showPrimarySelection = function () {
        // console.log("# ContentEditableInput.showPrimarySelection");
    ...
    ContentEditableInput.prototype.getCursorRange = function (from, to) {
        var cm = this.cm
        var start = posToDOM(cm, from);
        var end = posToDOM(cm, to);
        if (start && end) {
            var rng;
            try { return range(start.node, start.offset, end.offset, end.node); }
            catch (e) {
    ...
    ContentEditableInput.prototype.showMultipleSelections = function (info) {
        // console.log("# ContentEditableInput.showMultipleSelections");
    ...
    ContentEditableInput.prototype.focus = function () {
        // console.log("# ContentEditableInput.focus");
        if (this.cm.options.readOnly != "nocursor") {
            // console.log("# ContentEditableInput.focus!nocursor");
            if (!this.selectionInEditor()) {
                // console.log("# ContentEditableInput.focus!selectionInEditor");
    ...
    ContentEditableInput.prototype.receivedFocus = function () {
        // console.log("# ContentEditableInput.receivedFocus");
        var input = this;
    ...
    ContentEditableInput.prototype.pollSelection = function (force) {
        if (
            //#BEGIN HACK
            !force &&
            //#END HACK
...
XxXXC
        ...
        = new (function () {
            ...
            //#END
            var _editor = null;
            var _searchCursor = { from: { line: 0, ch: 0 }, to: { line: 0, ch: 0 } };
            var _focusListeners = [];
            var _touchstartListeners = [];
            function CSSEditorObject() {
                //#IF DEBUG
                //#ENDIF DEBUG
            }
            CSSEditorObject.prototype[_INIT] = function (text, fonts, selectionchanged) {
                //#IF DEBUG
                //#ENDIF DEBUG
                ...
                _editor = CodeMirror.fromTextArea(textarea, options);
                _editor.on(SELECTION_CHANGED, function () {
                    var cursor = _editor.getCursor();
                    var line = cursor ? cursor.line + 1 : null;
                    var ch = cursor ? cursor.ch + 1 : null;
                    //#IF DEBUG
                    //#ENDIF DEBUG
                    setTimeout(function() {
                        selectionchanged(line, ch);
                    }, 0);
                });
                function sinkhole(e) {
                    if (!e) return;
                ...
                if (doc) {
                    doc.addEventListener("dragstart", sinkhole);
                    doc.addEventListener("dragenter", sinkhole);
                    doc.addEventListener("dragover", sinkhole);
...
window.addEventListener("touchstart", function (e) {
                    ...
                    //#IF DEBUG
                    //#ENDIF DEBUG
                    _touchstartListeners.forEach(function (listener) { listener(); })
                });
                window.addEventListener("focus", function (e) {
                    if (_editor == null) return;
                    //#IF DEBUG
                    //#ENDIF DEBUG
                    _editor.ensureFocus1();
                    //        _editor.state.focused = true;
                    //        _editor.display.input.receivedFocus();
                    _focusListeners.forEach(function (listener) { listener(); })
                });
                window.addEventListener("blur", function (e) {
                    ...
                    //#IF DEBUG
                    //#ENDIF DEBUG
                    _searchCursor = { from: cursor, to: cursor };
                });
            }
            //#IF TESTING
            //#ENDIF TESTING
            CSSEditorObject.prototype[_BROWSE] = function () {
                if (_editor == null) return;
                //#IF DEBUG
                //#ENDIF DEBUG
                _editor.setOption("readOnly", "nocursor");
            }
            CSSEditorObject.prototype[_EDIT] = function () {
                if (_editor == null) return;
                //#IF DEBUG
                //#ENDIF DEBUG
                _editor.setOption("readOnly", false);
            }
            CSSEditorObject.prototype[_REVERT] = function (content) {
                if (_editor == null) return;
                //#IF DEBUG
                //#ENDIF DEBUG
            ...
            CSSEditorObject.prototype[_FOCUS] = function (listener) {
                //#IF DEBUG
                //#ENDIF DEBUG
                _focusListeners.push(listener);
            }
            CSSEditorObject.prototype[_DOWN] = function (listener) {
                //#IF DEBUG
                //#ENDIF DEBUG
                _touchstartListeners.push(listener);
            }
            CSSEditorObject.prototype[_ONSCROLL] = function (listener) {
                //#IF DEBUG
                //#ENDIF DEBUG
                var scrollable = window.document.querySelector("div.CodeMirror-scroll");
            ...
            CSSEditorObject.prototype[_ONKEYPRESS] = function (listener) {
                //#IF DEBUG
                //#ENDIF DEBUG
                var scrollable = window.document.querySelector("div.CodeMirror-code");
                if(scrollable == null) return;
                scrollable.addEventListener("keypress", function (e) {
                    if (_editor == null) return;
                    //#IF DEBUG
                    //#ENDIF DEBUG
                    listener(e);
            ...
            CSSEditorObject.prototype[_SAVE] = function () {
                if (_editor == null) return "";
                //#IF DEBUG
                //#ENDIF DEBUG
                // _editor.toTextArea();
                // var ret = _editor.getTextArea().value;
                return _editor.getValue();
            }
            CSSEditorObject.prototype[_PARSE] = function (text) {
                if (_editor == null) return [];
                //#IF DEBUG
                //#ENDIF DEBUG
                var CSSParser =
...
XxXPo
                ...
                var issues = CSSParser[_ISSUES](node);
                var ret = [];
                for (var i = 0; i < issues.length; ++i) {
                    var issue = issues[i];
                    ret.push([issue.offset, issue.message]);
                }
                return [node, ret];
            }
            CSSEditorObject.prototype[_EXPR] = function (text) {
                if (_editor == null) return [];
                //#IF DEBUG
                //#ENDIF DEBUG
                var CSSParser =
...
XxXPo
            ...
            CSSEditorObject.prototype[_TEXT] = function (text) {
                if (_editor == null) return;
                //#IF DEBUG
                //#ENDIF DEBUG
                var cm = _editor;
                cm.operation(function () {
                    //        var op = cm.curOp;
                    //        op.selectionChanged = true;
                    cm.replaceSelection(text, null);
                });
            }
            CSSEditorObject.prototype[_GETSEL] = function () {
                if (_editor == null) return null;
                var ret = _editor.getSelection();
                //#IF DEBUG
                //#ENDIF DEBUG
                return ret;
            }
            CSSEditorObject.prototype[_SETSEL] = function (fromline, fromch, toline, toch) {
                if (_editor == null) return null;
                //#IF DEBUG
                //#ENDIF DEBUG
                var cm = _editor;
                ...
                cm.operation(function () {
                    if (fromch == null) fromch = 1;
                    if (toch == null) toch = cm.getLine(toline - 1).length + 1;
                    var from = Pos(fromline - 1, fromch - 1);
                    var to = Pos(toline - 1, toch - 1);
                    cm.setSelection(from, to, { scroll: false });
                    cm.scrollIntoView({ from: from, to: to }, window.innerHeight / 2);
                });
            }
            CSSEditorObject.prototype[_REDO] = function () {
                if (_editor == null) return;
                //#IF DEBUG
                //#ENDIF DEBUG
                _editor.redo();
            }
            CSSEditorObject.prototype[_UNDO] = function () {
                if (_editor == null) return;
                //#IF DEBUG
                //#ENDIF DEBUG
                _editor.undo();
            }
            CSSEditorObject.prototype[_SIZE] = function () {
                if (_editor == null) return;
                //#IF DEBUG
                //#ENDIF DEBUG
                return _editor.historySize();
            }
            CSSEditorObject.prototype[_GOTO] = function (line, ch, callback) {
                if (_editor == null) {
                    callback(null);
                    return;
                }
                ...
                cm.operation(function () {
                    ...
                    var lastcol = cm.doc.getLine(line);
                    if (line > lastline ) line = lastline;
                    if (lastcol != null && ch > lastcol.length) ch = lastcol.length;
                    var pos = Pos(line, ch);
                    cm.setSelection(pos, pos, { scroll: false });
                    cm.scrollIntoView({ from: pos, to: pos }, window.innerHeight / 2);
                    setTimeout(function () {
                        setTimeout(function () {
                            var cursor = cm.getCursor();
                            var from = Pos(cursor.line, cursor.ch - 1);
                            var to = Pos(cursor.line, cursor.ch + 1);
                            callback(cm.display.input.getCursorRange(from, to));
                        }, 0);
                    }, 100);
                });
            }
            ...
            //        var data = event.data;
            //        var editor = window.XRY;
            //        if (origin != "c://localhost") return;
            //        if(editor == null) return;
            //        //#IF DEBUG
            //        console.log("# origin: " + origin + ", data: " + data);
            //        //#ENDIF DEBUG
            //        editor.undo();
            //        setTimeout(function () {document.body.focus(); editor._editor.display.input.focus();}, 20);
            //    });

            CSSEditorObject.prototype[_FIND] = function (query, reverse, ignorecase, callback) {
                if (_editor == null) return;
                var Pos = CodeMirror.Pos;
                var cm = _editor;
                var doc = cm.doc;
                ...
                function matches(pos) {
                    //#IF DEBUG
                    //#ENDIF DEBUG
                    return reverse ? searchStringBackward(doc, query, pos, ignorecase)
                        : searchStringForward(doc, query, pos, ignorecase);
                }
                //        function find(pos, reverse) {
                //      var result = matches(reverse, doc.clipPos(reverse ? pos.from : pos.to))
                //      // Implements weird auto-growing behavior on null-matches for
                //      // backwards-compatiblity with the vim code (unfortunately)
                //      while (result && CodeMirror.cmpPos(result.from, result.to) == 0) {
                //        if (reverse) {
                //          if (result.from.ch) result.from = Pos(result.from.line, result.from.ch - 1)
                //          else if (result.from.line == doc.firstLine()) result = null
                //          else result = matches(reverse, doc.clipPos(Pos(result.from.line - 1)))
                //        } else {
                //          if (result.to.ch < doc.getLine(result.to.line).length) result.to = Pos(result.to.line, result.to.ch + 1)
                //          else if (result.to.line == doc.lastLine()) result = null
                //          else result = matches(reverse, Pos(result.to.line + 1, 0))
                //        }
                //      }
                // if (result) {
                //   pos = result
                ...
                // } else {
                //   var end = Pos(reverse ? doc.firstLine() : doc.lastLine() + 1, 0)
                //   pos = { from: end, to: end }
                //   return atOccurrence = false
                // }
                //      }
                //        function getselectionrange() {
                //        var sel = window.getSelection();
                //        var range = (sel && sel.rangeCount > 0) ? sel.getRangeAt(0) : null;
                //        console.log("# find: selection range=" + (range == null ? "null" : range.anchorNode));
                //        return range;
                //        }

                cm.operation(function () {
                    // var pos = { from: cm.getCursor(), to: cm.getCursor() };
                    var cursor = matches(reverse ? _searchCursor.from : _searchCursor.to);
                    if (!cursor) {
                        _searchCursor = { from: CodeMirror.Pos(cm.lastLine()), to: CodeMirror.Pos(cm.firstLine(), 0) };
                        //#IF DEBUG
                        //#ENDIF DEBUG
                        callback(null);
                        return;
                    ...
                    cm.setSelection(cursor.from, cursor.to, { scroll: false });
                    cm.scrollIntoView({ from: cursor.from, to: cursor.to }, window.innerHeight / 2);
                    setTimeout(function () {
                        setTimeout(function () {
                            callback(cm.display.input.getCursorRange(cursor.from, cursor.to));
                        }, 0);
                    }, 100);
                });
            }
...
= (function () {
    ...
    //#BEGIN REGION CSSParserNodeType
    ...
    //    NodeType[3] = "Ruleset";
    //    NodeType[4] = "Selector";
    //    NodeType[5] = "SimpleSelector";
    //    NodeType[6] = "SelectorInterpolation";
    //    NodeType[7] = "SelectorCombinator";
    //    NodeType[8] = "SelectorCombinatorParent";
    //    NodeType[9] = "SelectorCombinatorSibling";
    //    NodeType[10] = "SelectorCombinatorAllSiblings";
    //    NodeType[11] = "SelectorCombinatorShadowPiercingDescendant";
    //    NodeType[12] = "Page";
    //    NodeType[13] = "PageBoxMarginBox";
    //    NodeType[14] = "ClassSelector";
    //    NodeType[15] = "IdentifierSelector";
    //    NodeType[16] = "ElementNameSelector";
    //    NodeType[17] = "PseudoSelector";
    //    NodeType[18] = "AttributeSelector";
    //    NodeType[19] = "Declaration";
    //    NodeType[20] = "Declarations";
    //    NodeType[21] = "Property";
    //    NodeType[22] = "Expression";
    //    NodeType[23] = "BinaryExpression";
    //    NodeType[24] = "Term";
    //    NodeType[25] = "Operator";
    //    NodeType[26] = "Value";
    //    NodeType[27] = "StringLiteral";
    //    NodeType[28] = "URILiteral";
    //    NodeType[29] = "EscapedValue";
    //    NodeType[30] = "Function";
    //    NodeType[31] = "NumericValue";
    //    NodeType[32] = "HexColorValue";
    //    NodeType[33] = "MixinDeclaration";
    //    NodeType[34] = "MixinReference";
    //    NodeType[35] = "VariableName";
    //    NodeType[36] = "VariableDeclaration";
    //    NodeType[37] = "Prio";
    //    NodeType[38] = "Interpolation";
    //    NodeType[39] = "NestedProperties";
    //    NodeType[40] = "ExtendsReference";
    //    NodeType[41] = "SelectorPlaceholder";
    //    NodeType[42] = "Debug";
    //    NodeType[43] = "If";
    //    NodeType[44] = "Else";
    //    NodeType[45] = "For";
    //    NodeType[46] = "Each";
    //    NodeType[47] = "While";
    //    NodeType[48] = "MixinContent";
    //    NodeType[49] = "Media";
    //    NodeType[50] = "Keyframe";
    //    NodeType[51] = "FontFace";
    //    NodeType[52] = "Import";
    //    NodeType[53] = "Namespace";
    //    NodeType[54] = "Invocation";
    //    NodeType[55] = "FunctionDeclaration";
    //    NodeType[56] = "ReturnStatement";
    //    NodeType[57] = "MediaQuery";
    //    NodeType[58] = "FunctionParameter";
    //    NodeType[59] = "FunctionArgument";
    //    NodeType[60] = "KeyframeSelector";
    //    NodeType[61] = "ViewPort";
    //    NodeType[62] = "Document";
    //    NodeType[63] = "AtApplyRule";
    //    NodeType[64] = "CustomPropertyDeclaration";
    //    NodeType[65] = "CustomPropertySet";
    //    NodeType[66] = "ListEntry";
    //    NodeType[67] = "Supports";
    //    NodeType[68] = "SupportsCondition";
    //    NodeType[69] = "NamespacePrefix";
    //    NodeType[70] = "GridLine";
    //    NodeType[71] = "Plugin";
    //    NodeType[72] = "UnknownAtRule";
    //    CSSParserObject[_NODETYPE] = function (id) {
    //        return NodeType[id];
    //    };
...
(function (mod) {
    ...
    CodeMirror.defineMode("css", function (config, parserConfig) {
        ...
        function tokenBase(stream, state) {
            ...
            if (ch == "@") {
                ...
                stream.eatWhile(/[\w\\\-]/);
                return ret("number", "hash"); // HACK original atom
            } else if (ch == "!") {
                stream.match(/^\s*\w*/);
                return ret("keyword-2", "important");
    ...
    function _cssproperties() {
        var upper = /[A-Z]/g;
        var names = Object.keys(document.body.style);
        return names.map(function (name) {
            var ret = name.replace(upper, function (c) {
                return "-" + c.toLowerCase();
            });
            if (ret.indexOf("webkit-") != 0) return ret;
            return "-" + ret;
        });
    }
    ...
    //  ],
    var propertyKeywords_ = _cssproperties(),
...
(function (mod) {
    ...
    var myPropertyKeywords_ = null;
    var myValueKeywords_ = null;
    var myFontNames_ = null;
    var lcFontNames_ = null;
    ...
    function myPropertyKeywords(spec) {
        if (myPropertyKeywords_ == null) {
            myPropertyKeywords_ = keySet([":", "-", "name value;", "} {"]
                .concat(Object.keys(spec.propertyKeywords).map(function (s) {
                    return s + ": ";
                })));
        }
        return myPropertyKeywords_;
    }

    function initFontNames(cm) {
        var names = [];
        var lc = [];
        var fonts = cm.getOption("fonts");
        if (fonts) {
        ...
        myFontNames_ = keySet(names);
        lcFontNames_ = keySet(names.map(function (name) {
    ...
    function myValueKeywords(spec) {
        if (myValueKeywords_ == null) {
            myValueKeywords_ = keySet([";", ", ", "\"\"", "()", "!important", "#fff"]
                .concat(Object.keys(spec.valueKeywords).map(function (name) {
                    var value = hintmap[name];
                    return value ? value : name;
                }))
                .concat(Object.keys(spec.colorKeywords)));
        }
        return myValueKeywords_;
    }

    function myFontNames(cm) {
        if (myFontNames_ == null) {
            initFontNames(cm);
        }
        return myFontNames_;
    }

    function lcFontNames(cm) {
        if (lcFontNames_ == null) {
            initFontNames(cm);
        }
        return lcFontNames_;
    }
    ...
    //#BEGIN NOT_USED
    //    var myValueKeywords_ = null;
    //    var myColorKeywords_ = null;
    //
    //  function myValueKeywords(spec) {
    //    if (myValueKeywords_ == null) {
    //        myValueKeywords_ = keySet(Object.keys(spec.valueKeywords).map(function(s) { return s + ";"; }));
    //    }
    //    return myValueKeywords_;
    //  }
    //
    //  function myColorKeywords(spec) {
    //    if (myColorKeywords_ == null) {
    //        myColorKeywords_ = keySet(Object.keys(spec.colorKeywords).map(function(s) { return s + ";"; }));
    //    }
    //    return myColorKeywords_;
    //  }
    //#END NOT_USED
    ...
    CodeMirror.registerHelper("hint", "css", function (cm) {
        ...
        if (token.type == "keyword-2" && "!important".indexOf(token.string) == 0)
            return {
                list: ["!important"],
        ...
        var lcword = word ? word.toLowerCase() : word;
        var result = [];
        ...
        function fuzzymatch(search, text) {
            ...
            for (var i = 0; i < search.length; ++i) {
                ...
                var index = text.indexOf(c, start);
                if (index < 0) {
                    return false;
                }
                start = index + 1;
        ...
        function add11(name) {
            ...
            if (!lcword || fuzzymatch(lcword, name)) {
                result.push(name);
            }
        }

        function add2(keywords, lckeywords) {
            var keys = Object.keys(lckeywords);
            var index = 0;
            for (var name in keywords) {
                add21(name, keys[index]);
                index += 1;
            }
        }

        function add21(name, lcname) {
            // if (!word || name.lastIndexOf(word, 0) == 0) {
            if (!lcword || fuzzymatch(lcword, lcname)) {
                result.push(name);
            }
        }
        ...
        if (st == "pseudo" || token.type == "variable-3") {
            //      if (word && pseudoClasses.hasOwnProperty(word)) {
            //        add1(word);
            //      } else {
            add1(pseudoClasses);
            //      }
        } else if (st == "block" || st == "maybeprop") {
            //        if (word && (myPropertyKeywords(spec).hasOwnProperty(word))) {
            //            add1(word);
            //        } else {
            add1(myPropertyKeywords(spec));
            //      }
        } else if (st == "prop" || st == "parens" || st == "at" || st == "params") {
            //      if(word &&
            //          (spec.valueKeywords.hasOwnProperty(word)
            //          || spec.colorKeywords.hasOwnProperty(word))) {
            //           add1(word);
            //      } else {
            add1(myValueKeywords(spec));
            add2(myFontNames(cm), lcFontNames(cm));
            //          add(spec.valueKeywords);
            //          add(spec.colorKeywords);
            //      }
        } else if (st == "media" || st == "media_parens") {
            //      if (word &&
            //        (spec.mediaTypes.hasOwnProperty(word)
            //        || spec.mediaFeatures.hasOwnProperty(word))) {
            //        add1(word);
            //      } else {
            add1(spec.mediaTypes);
            add1(spec.mediaFeatures);
            //      }
        }
