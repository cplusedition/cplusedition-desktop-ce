...
class EtUt {
    ...
    static configSession_(session, log = null) {
        session
            ...
            log === null || log === void 0 ? void 0 : log.d_(`# WARN: Deny permssion: ${permission} for ${url}`);
            callback(false);
        });
        session.on('will-download', (event, item, _webContents) => {
            const url = item.getURL();
            const mime = item.getMimeType();
            log === null || log === void 0 ? void 0 : log.d_(`# WARN: Deny download: ${url}, ${mime}`);
            event.preventDefault();
            item.cancel();
        });
        session.on('spellcheck-dictionary-download-begin', (event, langcode) => {
            log === null || log === void 0 ? void 0 : log.d_(`# WARN: Deny download dictionary for: ${langcode}`);
            event.preventDefault();
        });
